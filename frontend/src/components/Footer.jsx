      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="md:col-span-2">
            <img src={logoLight} alt="DA SHOP" className="h-9 w-auto mb-5" />
          ...
          {/* Categories */}
          <div>
            <p className="text-white/30 text-xs tracking-widest uppercase font-semibold mb-5">Categories</p>
            <ul className="space-y-3">
              {categories.map((cat) => (
                <li key={cat}>
                  <Link to={`/?category=${cat}`} className="text-white/40 hover:text-white text-sm transition-colors">
                    {cat}
                  </Link>
                </li>
              ))}
          ...
          {/* Vendors */}
          <div>
            <p className="text-white/30 text-xs tracking-widest uppercase font-semibold mb-5">Vendors</p>
            <ul className="space-y-3">
              {vendors.map((vendor) => (
                <li key={vendor.id}>
                  <Link to={`/vendor/${vendor.id}`} className="text-white/40 hover:text-white text-sm transition-colors">
                    {vendor.name}
                  </Link>
                </li>
              ))}
