      ...
      <section className="max-w-[800px] mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-rule">
          ...
          <div className="bg-white p-8">
            <p className="text-ink text-[10px] tracking-[0.2em] uppercase font-black mb-6">
              Items Ordered
            </p>
            <div className="flex flex-col gap-4">
              {(Array.isArray(order?.items) ? order.items : []).map((item, i) => (
                <div key={i} className="flex justify-between items-start gap-4">
                  <div>
                    ...
                    <p className="text-ink font-bold text-sm">{item.product_name}</p>
                    {(item.variant || item.size) && (
                      <p className="text-mute text-[11px]">
                        {item.variant && <>Color: <span className="text-ink">{item.variant}</span></>}
                        {item.variant && item.size && <span className="mx-1.5">·</span>}
                        {item.size && <>Size: <span className="text-ink">{item.size}</span></>}
                      </p>
                    )}
                    <p className="text-mute text-xs">×{item.quantity}</p>
                  </div>
              ...
              <div className="pt-4 border-t border-rule flex justify-between">
                <span className="text-ink text-[11px] tracking-[0.15em] uppercase font-black">Total</span>
                <span className="text-ink font-black text-lg">${Number(order?.total ?? 0).toFixed(2)}</span>
              </div>
            </div>
          </div>
        ...
        {isGuest && (
          <div className="mt-12 bg-white border border-rule p-8">
            <p className="text-ink text-[10px] tracking-[0.2em] uppercase font-black mb-3">
              Become a Member
            </p>
            ...
            <Link
              ...
              className="inline-block bg-ink text-white font-black text-[11px] tracking-[0.15em] uppercase px-8 py-4 hover:bg-ink/80 transition-colors"
            >
              Create Account
            </Link>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-12">
          <Link
            to="/"
            className="flex-1 bg-ink text-white font-black text-[11px] tracking-[0.15em] uppercase py-4 text-center hover:bg-ink/80 transition-colors"
          >
            Continue Shopping
          </Link>
          {!isGuest && (
            <Link
              to="/profile"
              className="flex-1 border border-ink text-ink font-black text-[11px] tracking-[0.15em] uppercase py-4 text-center hover:bg-ink hover:text-white transition-colors"
            >
              View My Orders
            </Link>
          )}
