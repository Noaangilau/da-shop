  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="max-w-[900px] mx-auto px-6 py-20">
        ...
        <p className="text-mute text-sm mb-16">Ships from New Zealand · All prices in NZD</p>

        {/* Rates table */}
        <div className="mb-20">
          <h2 className="text-ink font-black uppercase tracking-wide text-sm mb-6 border-t border-rule pt-8">
            Shipping Rates
          </h2>
            ...
            <table className="w-full text-sm border border-rule">
              <thead>
                <tr className="bg-paper border-b border-rule">
                  <th className="text-left px-5 py-3 text-[10px] tracking-[0.2em] uppercase text-mute font-semibold">Zone</th>
                  <th className="text-left px-5 py-3 text-[10px] tracking-[0.2em] uppercase text-mute font-semibold">Standard</th>
                  <th className="text-left px-5 py-3 text-[10px] tracking-[0.2em] uppercase text-mute font-semibold">Express</th>
                  <th className="text-left px-5 py-3 text-[10px] tracking-[0.2em] uppercase text-mute font-semibold">Free Shipping</th>
                </tr>
              </thead>
              <tbody>
                {RATES.map((r, i) => (
                  <tr key={r.zone} className={`border-b border-rule ${i % 2 === 0 ? 'bg-white' : 'bg-paper/50'}`}>
                    <td className="px-5 py-4 text-ink font-bold text-[13px]">{r.zone}</td>
                    <td className="px-5 py-4 text-gray-500 text-[13px]">
                      <div>{r.standardCost}</div>
                      <div className="text-[11px] text-mute mt-0.5">{r.standard}</div>
                    </td>
                    <td className="px-5 py-4 text-gray-500 text-[13px]">
                      {r.expressCost !== '—' ? (
                        <>
                          <div>{r.expressCost}</div>
                          <div className="text-[11px] text-mute mt-0.5">{r.express}</div>
                        </>
                      ) : (
                    ...
                    <td className="px-5 py-4 text-gray-500 text-[13px]">
                      {r.free ? <span className="text-ink font-semibold text-[12px]">{r.free}</span> : <span className="text-mute">—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
